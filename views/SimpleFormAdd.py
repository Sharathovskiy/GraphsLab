...
class AddGraphSimpleFormViewCreator:
    def __init__(self, master):
        ...
        self.edge_label = Label(master, text="Edge label")
        ...
        self.edge_label_entry = Entry(master)
        ...
        self.node_weight_label.grid(row=2, sticky=E)
        self.add_node_button.grid(row=3, columnspan=3)
        self.current_nodes_label.grid(row=4)
        self.current_nodes_info.grid(row=4, column=1, sticky=W)

        # ENTRIES
        self.add_node_entry.grid(row=0, column=1)
        self.node_label_entry.grid(row=1, column=1)
        self.node_weight.grid(row=2, column=1)

        # EDGE COLUMN

        # LABELS
        self.add_edge_label.grid(row=0, column=2, sticky=E)
        self.edge_label.grid(row=1, column=2, sticky=E)
        self.edge_weight_label.grid(row=2, column=2, sticky=E)
        self.add_edge_button.grid(row=3, column=3, columnspan=3)
        self.current_edges_label.grid(row=4, column=2)
        self.current_edges_info.grid(row=4, column=3, sticky=W)

        # ENTRIES
        self.add_edge_entry.grid(row=0, column=3)
        self.edge_label_entry.grid(row=1, column=3)
        self.edge_weight.grid(row=2, column=3)
    ...
    def add_node(self):
        ...
        self.graphService.add_node(node, label=label, weight=weight)
        print(self.graphService.graph.node[node]['label'])
    ...
    def add_edge(self):
        ...
        self.graphService.add_edge(u, v, label=label, weight=weight)
        print(self.graphService.graph[u][v]['label'])

        self.erase_entry(self.add_edge_entry)
        self.erase_entry(self.edge_label_entry)
